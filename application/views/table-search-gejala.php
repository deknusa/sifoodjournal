<script language= "javascript" >
    function lihat(KodeGejala){
      window.open("<?php echo base_url()?>cSearchGejala/lihat/"+KodeGejala,"_self");
    }
    function lihatJum(NamaMakan,Gejala){
      window.open("<?php echo base_url()?>cSearchGejala/lihatJum/"+NamaMakan+"/"+Gejala,"_self");
    }
    function ditail(KodeMakan){
      window.open("<?php echo base_url()?>cSearchGejala/ditail/"+KodeMakan,"_self");
    }
</script>
<div class="container">
    <?php if (!empty($hasil1['gejala'])) { ?>
        ...
        <table class="table">
            ...
            <tbody>
                <?php foreach ($hasil1['gejala'] as $data) { ?>
                    <tr>
                        <td><?php echo $data['Gejala']; ?></td>
                        <td><?php echo $data['Waktu']; ?></td>
                        <td><button type="button" class="btn btn-primary btn-sm" onclick="lihat('<?php echo $data['KodeGejala']?>')">Lihat</button></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
        ...
        <table class="table">
            ...
            <tbody>
                <?php foreach ($hasil1['hitung'] as $data) {?>
                    <?php foreach ($hasil1['gejala'] as $a) { ?>
                        <?php  $a['Gejala']; ?></td>
                    <?php } ?>
                    <tr>
                        <td><?php echo $data['NamaMakan']; ?></td>
                        <td><?php echo $data['jumlah']; ?></td>
                        <td><button type="button" class="btn btn-primary btn-sm" onclick="lihatJum('<?php echo $data['NamaMakan']?>','<?php echo $a['Gejala'] ?>')">Lihat</button></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
    ...
    <?php } else if(!empty($hasil2['makan'])){ ?>
        ...
        <table class="table">
            ...
            <tbody>
                <?php foreach ($hasil2['mood'] as $data) { ?>
                    <tr>
